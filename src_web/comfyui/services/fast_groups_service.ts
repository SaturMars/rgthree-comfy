...
class FastGroupsService {
  ...
  private run() {
    ...
    for (const node of this.fastGroupNodes) {
      // A throw here would otherwise leave `runScheduledForMs` set with nothing scheduled,
      // permanently killing the sync loop and freezing whatever half-refreshed toggle
      // states existed (seen with ComfyUI's Nodes 2.0 widget machinery). Keep polling so
      // the widgets re-sync on the next run, and surface the error.
      try {
        node.refreshWidgets();
      } catch (err) {
