...
export abstract class RgthreeBaseNode extends LGraphNode {
  ...
  constructor(title = RgthreeBaseNode.title, skipOnConstructedCall = true) {
    ...
    // Newer ComfyUI frontends define `mode` as a prototype accessor backed by a reactive
    // state object that the renderer and the serializer read directly. An own property
    // that only stores to a private field diverges from that store: a muted rgthree node
    // (like Context) still renders and serializes as active, so its group flips back on
    // after the workflow is reloaded. Route reads/writes through the base class'
    // accessor when there is one; older frontends keep the private-field behavior.
    const baseModeDesc = Object.getOwnPropertyDescriptor(LGraphNode.prototype, "mode");
    defineProperty(this, "mode", {
      get: () => {
        return baseModeDesc?.get ? baseModeDesc.get.call(this) : this.rgthree_mode;
      },
      set: (mode: LGraphEventMode) => {
        const currentMode = baseModeDesc?.get ? baseModeDesc.get.call(this) : this.rgthree_mode;
        if (currentMode != mode) {
          if (baseModeDesc?.set) {
